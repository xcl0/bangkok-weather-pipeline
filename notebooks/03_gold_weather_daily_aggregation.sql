...
-- Identify dates that contain fewer than 24 hourly weather records
SELECT
    weather_date,
    recorded_hours
FROM workspace.default.gold_weather_daily
WHERE recorded_hours < 24
ORDER BY weather_date;


-- COMMAND ----------

-- Check that the daily average temperature falls between the minimum and maximum
SELECT *
FROM workspace.default.gold_weather_daily
WHERE min_temperature_celsius > avg_temperature_celsius
   OR avg_temperature_celsius > max_temperature_celsius;

-- COMMAND ----------

-- Check that the stored temperature range matches maximum minus minimum
SELECT *
FROM workspace.default.gold_weather_daily
WHERE temperature_range_celsius
      != ROUND(
